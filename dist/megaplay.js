...
var MEGAPLAY_CDN_TOKEN_KEY = "MpCdnT0k3n!9f2K#xQ7vL5mR8wN1pY4s";
var MEGAPLAY_CDN_TOKEN_TTL_SECONDS = 120;
function base64UrlEncode(input) {
  return Buffer.from(input).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
function isNexabloomMasterUrl(url) {
  try {
    const parsed = new URL(url);
    return parsed.hostname.toLowerCase().endsWith("nexabloom.top") && parsed.pathname.includes("master.m3u8");
  } catch {
    return false;
  }
}
function signNexabloomMasterUrl(masterUrl, ttlSeconds = MEGAPLAY_CDN_TOKEN_TTL_SECONDS) {
  ...
  const signature = base64UrlEncode(
    (0, import_crypto.createHmac)("sha256", MEGAPLAY_CDN_TOKEN_KEY).update(message).digest()
  );
